 ...
 * @LastEditors: monai
 * @LastEditTime: 2020-04-20 15:48:51
 */
...
module.exports= merge(require("./webpack.configBase.js"), {
    ...
    output: {
        ...
        filename: `${base.buildJS + path.sep}[name].[contenthash].js`,
        //  chunk文件输出名称
        chunkFilename: `${base.buildJS + path.sep}[name].chunk.[contenthash].js`,
    },
    module: {
        rules:[
            {
                ...
                use:[
                    {
                        ...
                        options: {
                            //  css 单独抽离之后，被放置在 css 文件夹，图片被 loder 处理到 img 文件夹中，css img 文件夹同级，需要返回到同级去加载
                            publicPath: '../'
                        }
                    },
                    {
    ...
    plugins: [
        ...
        new MiniCssExtractPlugin({
            ...
            filename: `${base.buildCSS + path.sep}[name].[contenthash].css`,
            chunkFilename: `${base.buildCSS + path.sep}chunk.[name].[contenthash].css`
        }),
        // dllPlugin关联配置
